...
const htmlPath = path.join(__dirname, '../assets/bmi-health-calculator.html');
const jsPath = path.join(__dirname, '../assets/bmi-health-calculator.js');
...
const inlineScript = `
    <script type="module">
      ...
      import(url)
        .catch(err => {
          console.error('[BMI Calculator] Failed to load:', err);
          const root = document.getElementById('bmi-health-calculator-root');
          if (root) {
